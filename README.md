...
>    {$match :{ApplicantIncome : {$gt : 2000}}},  <br>
>    {$count :"total Number of Applicants who have ApplicantIncome > 2000 are"} <br>
>  ])

So total count given by above query is : 572
<h3>Output :</h3>
...
For my Collection :
The following example creates a single key Increasing index on the Loan_ID field: 

> db.user_details.createIndex({Loan_ID : 1})

<p>The default name for an index is the concatenation of the indexed keys and each key's direction in the index ( i.e. 1 or -1) using underscores as a separator. For example, an index created on { Loan_ID : 1 } has the name Loan_ID_1. </p>

<p>You can create indexes with a custom name, such as one that is more human-readable than the default. The following createIndex() method creates an index on Loan_Amount_Term and Property_Area in Compound Index form and Renames index name using "name" field as "Compound Index".</p>

> db.user_details.createIndex({    <br>
>  Loan_Amount_Term : 1, Property_Area : -1   <br> 
> }, {name :"Compound Index"})  

Here searching will be based on increasing order of Loan_Amount_Term field and then for each Loan_Amount_Term there will be decreasing level search for Property_Area Field Value.<br>

> db.user_details.createIndex({CoapplicantIncome: -1})

This will give index to the CoapplicantIncome field and searching will be carried out in decreasing manner. <br>
